...
def generar_rutas_planificadas(json_path, matriz_tiempos_path, lat_ollauri=42.539, lon_ollauri=-2.848):
    ...
    def agregar_tramos_a_csv(semana, dia, fecha, ruta, matriz, tecnico_id, df):
        # Tiempo desde Ollauri al primer destino
        primer_destino = ruta[1] if len(ruta) > 1 else "ollauri"
        try:
            tiempo_salida = float(matriz.at['ollauri', primer_destino])
        except KeyError:
            tiempo_salida = 0.0
        ...
        rutas_para_csv.append({
            ...
            'Horas': 0.0, 'Tiempo_Desplazamiento_min': tiempo_salida,
    ...
    return df_rutas.to_dict(orient='records')
